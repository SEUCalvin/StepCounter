...
/**
 * Created by Calvin on 2021/7/22.
 * 加速度传感器计算当天步数，需要保持后台Service
 */
public class StepSystemAccelerometer implements SensorEventListener {
    ...
    public StepSystemAccelerometer(Context context, StepCounterListener stepCounterListener) {
        super();
        mContext = context;
        ...
        mCount = (int) PreferencesUtil.getCurrentStep(mContext);
    ...
    private synchronized void dateChangeCleanStep() {
        ...
        if (!getTodayDate().equals(mTodayDate)) {
            ...
            mCount = 0;
            PreferencesUtil.setCurrentStep(mContext, mCount);
    ...
    private void updateStepCounter() {
        ...
        if (mStepCounterListener != null) {
            mStepCounterListener.onChangeStepCounter(mCount);
        }
    }
    ...
    //连续走十步才会开始计步 连续走了9步以下, 停留超过3秒, 则计数清空
